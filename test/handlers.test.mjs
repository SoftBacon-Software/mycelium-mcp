...
routes['GET /echo'] = (req, res) => json(res, 200, { agentKey: req.headers['x-agent-key'] || null });
...
routes['PUT /tasks/7'] = (req, res, body) => {
  assert.equal(body.status, 'done');
  json(res, 200, { ok: true });
};
routes['GET /work/test-agent'] = (req, res) => json(res, 200, {
  ok: true,
  queue: [{ type: 'task', id: 9, title: 'Next thing', status: 'open' }]
});
routes['POST /agents/heartbeat'] = (req, res) => json(res, 200, { ok: true, pending: 0 });
...
routes['GET /work/test-agent'] = (req, res) => json(res, 200, { ok: true, queue: [] });
...
routes['GET /tasks/404'] = (req, res) => json(res, 404, { error: 'Task not found' });
...
const bootResult = await new Promise((resolvePromise) => {
  ...
  const timer = setTimeout(() => resolvePromise({ ok: false, why: 'no startup line in 10s', stderr }), 10000);
...
assert.ok(bootResult.ok, 'index.js must start and exit 0 on SIGTERM — ' + bootResult.why + '\n' + bootResult.stderr);
...
assert.ok(orphanResult.ok, 'index.js must exit when stdin closes — ' + orphanResult.why + '\n' + orphanResult.stderr);
